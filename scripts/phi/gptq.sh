...
BASE_PLOT_PATH="plots/alpha/phi/3bits_g128_fix"
...
for wbits in "${WBITS_VALUES[@]}"; do
  ...
  for seed in "${SEEDS[@]}"; do
    ...
    LOG_FILE="logs/phi/${DATE}_${wbits}bit-g128_gptq_seed${seed}.log"
    
    CMD="CUDA_VISIBLE_DEVICES=0 $VENV/bin/python -u phi.py \
      $MODEL_PATH c4 \
      --method gptq \
      --sym \
      --wbits $wbits \
