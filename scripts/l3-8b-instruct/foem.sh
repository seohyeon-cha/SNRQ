#!/bin/bash
#SBATCH -J foem        # Job name
#SBATCH -p gh             # Partition (queue) name
#SBATCH -N 1              # Total number of nodes
#SBATCH -n 1              # Total number of MPI tasks
#SBATCH -t 2:00:00     
#SBATCH --output=slurm_out/foem_%j.out
#SBATCH --error=slurm_out/foem_%j.err
...
for wbits in "${WBITS_VALUES[@]}"; do
  ...
  for seed in "${SEEDS[@]}"; do
    ...
    LOG_FILE="/work/10322/scha0901/vista/FOEM/LLM/weight-only/logs/l3-8b-instruct/${DATE}_l3-8b-instruct-${wbits}bit-128g_foem_seed${seed}.log"
    ...
    CMD="CUDA_VISIBLE_DEVICES=0 $VENV/bin/python -u /work/10322/scha0901/vista/FOEM/LLM/weight-only/llama_step.py \
      ...
      --lm-eval \
      --wandb \
      --wandb-project L3-8B-Instruct-symm-new \
