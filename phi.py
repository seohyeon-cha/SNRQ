"""Reference: https://github.com/jerry-chee/DiscQuant/blob/main/gptq/phi3.py """
...
from transformers import AutoModelForCausalLM, PhiConfig, PhiForCausalLM, modeling_utils
...
def get_phi(name):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        name, torch_dtype='auto', trust_remote_code=True, attn_implementation="eager")
    model.seqlen = 2048
    return model

@torch.no_grad()
def phi_sequential(model, dataloader, dev, fp_path):
    print('Starting ...')
...
if __name__ == '__main__':
    ...
    if args.lm_eval:
        ...
        # Fix for DynamicCache.get_usable_length compatibility issue
        # Disable use_cache to avoid the get_usable_length error
        use_cache_original = model.config.use_cache
        model.config.use_cache = False
        
        try:
            # Try importing DynamicCache from different possible locations
            try:
                from transformers.cache_utils import DynamicCache
            except ImportError:
                try:
                    from transformers.modeling_utils import DynamicCache
                except ImportError:
                    # If DynamicCache is not found, skip patching
                    DynamicCache = None
            ...
            # If patching fails, continue anyway - use_cache=False should prevent the error
            pass
